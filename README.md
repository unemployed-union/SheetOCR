...
* 가로 지그재그로 자르면 문맥을 파악하기 힘들 정도로 멀리 떨어져있게 되므로, 아무리 글자를 크게 자른다 한들 문맥 파악이 힘들어 제대로 글자를 추론하기 힘듦
  * 반면 세로로 자르게 된다면 인식해야 하는 글자의 정보가 가까운 위치에 존재하게 되므로, 문맥 파악이 쉬워져 글자를 읽을 수 있게 됨 (왼쪽에서 오른쪽으로 흐르는 시계열 데이터가 됨)
